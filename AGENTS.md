...
- `overlapTokens`: reserved for future overlap strategy (currently not active in runtime loop)
...
1. **Explicit override via marker in chunk output**
   ...
   - accept if meaningful length
2. **LLM-based compression**
   - call injected `compressorFn(prompt, systemPrompt)`
   - include previous state + current chunk preview
   - enforce concise "3-5 critical points" style
...
- `historySize = i * chunkSize` (reference estimator)
...
## 14) Notes from Current Source of Truth

- Token accounting is heuristic (`chars / 4`), not tokenizer-precise.
- `overlapTokens` exists in config contract but is not currently used in runtime prompt construction.
- Output validation type definitions exist but are not yet wired into run-time control flow.
- Historical "actual performance" currently computes Markovian step cost as `carryoverTokens + chunk.tokens` using current config; teams may choose to persist per-run config snapshot for stricter historical fidelity.

This is intentional to keep the implementation resilient, lightweight, and provider-portable.
