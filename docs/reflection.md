...
Three modes, picked at config time:
...
The host can accept the refinement, accept the original, or escalate.

### Mode 3 — Branch and converge

Reflection spawns a small second chain that addresses the issues, then merges with the original. More expensive; useful when refinement requires actual work, not just rewording.
