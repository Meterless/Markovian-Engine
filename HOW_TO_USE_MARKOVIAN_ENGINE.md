...
| `maxChunks` | 12 | 1 | — | 1 |
| `carryoverTokens` | 256 | 128 | 32768 | 128 |
| `overlapTokens` | 128 | 0 | — | — |

`overlapTokens` lives in the config contract but is not active in the current runtime loop. Reserve it for a future overlap strategy.
...
async function extractCarryover(prevState: string, chunkOutput: string): Promise<string> {
  ...
  if (marker && marker.length >= 20) return sanitize(marker);
...
pub async fn extract_carryover(prev_state: &str, chunk_output: &str, cfg: &ChunkConfig, compressor: &dyn Compressor) -> String {
    if let Some(marker) = parse_state_block(chunk_output) {
        if marker.len() >= 20 { return sanitize(&marker); }
    }
...
- Persist the **config snapshot** alongside the run if you want strict historical efficiency fidelity. The reference uses current config when recomputing, which is intentionally lightweight.
...
function estimateSavings(i: number, currentText: string, cfg: ChunkConfig) {
  ...
  const historySize = i * cfg.chunkSize;
...
# Python
import math
def estimate_savings(i, current_text, cfg):
    chunk_tokens = math.ceil(len(current_text) / 4)
    history_size = i * cfg.chunk_size
...
pub fn estimate_savings(i: u32, current_text: &str, cfg: &ChunkConfig) -> Savings {
    ...
    let history_size = i * cfg.chunk_size;
...
Token accounting in the reference is `ceil(chars / 4)`. For tokenizer-precise accounting, swap in your provider's tokenizer.
...
async function reflect(run: MarkovianRun) {
  if (!run.completed || run.chunks.length === 0) return;
...
The companion reference implementation lives in the `meterless-markovian-engine/` repository. It is documentation-first: the engine contracts are specified before the code, so the layout maps concepts.
...
│   ├── chunk-config.md          tokensPerChunk / carryoverTokens sizing
