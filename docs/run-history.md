...
type Run = {
  ...
  startedAt: ISODate;
  endedAt: ISODate;
  status: "completed" | "max-chunks" | "errored" | "paused";
};
...
- Parsed markers (CARRYOVER, PROGRESS, DONE, NEEDS_TOOL, NEEDS_CLARIFICATION)
- Token counts on both sides
- Latency per chunk
- Tool calls if any
- Errors and warnings
...
By default runs are stored in local storage (IndexedDB in browser, filesystem in Node). Configurable:
...
```ts
const run = await markovian.runs.get(runId);
