...
|---|---|---|---|
| `tokensPerChunk` | Per-call budget | Bigger model, complex steps | Smaller model, simpler steps |
| `carryoverTokens` | State across chunks | Decision-heavy, state-heavy work | Information-light, creative work |
| `framingTokens` | Goal + system prompt | Complex protocol requirements | Stable, simple framing |
| `overlapTokens` | Continuity hint | Long-form writing, code | Most tasks (keep at 0) |

## Step 1 — Profile your task (10 min)

Three questions:

1. **What's the average per-step output?** If steps produce ~500 tokens, your `tokensPerChunk` can be smaller. If steps produce ~3000 tokens, you need headroom.
2. **What state has to cross?** Decisions, entities, open questions — pin them. Worked examples — drop them.
3. **What model are you using?** Set `tokensPerChunk` to 60-75% of the context window.
...
```ts
const configs = [
  { label: "tight", chunkConfig: { tokensPerChunk: 4000, carryoverTokens: 400 } },
  { label: "default", chunkConfig: { tokensPerChunk: 6000, carryoverTokens: 800 } },
  { label: "loose", chunkConfig: { tokensPerChunk: 10000, carryoverTokens: 1600 } },
];
