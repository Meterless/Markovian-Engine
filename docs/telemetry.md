...
run.status                 completed | max-chunks | errored | paused
...
For long runs this number is often 80-95%. For short ones (3-5 chunks) it's 30-50%. See [`efficiency-model.md`](./efficiency-model.md) for the derivation and worked examples.
...
**2. Local dashboard** — opt-in.

```bash
npm run markovian:dashboard
# → http://localhost:5176
```
