...
```text
1. Explicit marker override   parse <CARRYOVER>; accept if meaningful length
...
const markovian = new Markovian({
  chunkConfig: { tokensPerChunk: 5000, carryoverTokens: 600 },
...
- **A missing marker is not a failure.** Strategy 3 (model compression) catches the common "model forgot the marker" case automatically. The chain keeps its O(1) property even when the model is sloppy.
- **The cascade is robust under model/API instability.** If the compressor call itself errors, strategy 3 falls to the deterministic regex extractor, then to a hard tail-truncate. There is always a bounded carryover — the run cannot wedge.
